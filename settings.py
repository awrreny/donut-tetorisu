...
import perfect_clear_info as pci
...
key_soft_drop = pgl.K_DOWN
key_move_left = pgl.K_LEFT
key_move_right = pgl.K_RIGHT
key_hard_drop = pgl.K_SPACE
key_90_clockwise = pgl.K_d
key_90_anticlockwise = pgl.K_a
key_180 = pgl.K_s
key_hold = pgl.K_BACKSLASH
key_hold2 = pgl.K_LSHIFT
key_printboard = pgl.K_b
key_changeCustomBag = pgl.K_n  # to input your own queue
key_changePreset = pgl.K_m  # to input a board based on presets.py
key_resetboard = pgl.K_F4  # window loses focus when you do this (but only once?)
key_retryqueue = pgl.K_4  # does the same as resetboard but with the same queue (useful for retrying pc solutions)
...
tekoFontVisible = False  # change this to False if there is lots of text overlapping
...
def create_very_custom_queue():
    ...
    #    if pci.extra_pieces_to_pc_num(extra_pieces - 1, 5) % 7 <= 3:
    #        continue
    #    print(f"extra_pieces: {extra_pieces - 1}  nth pc: {pci.extra_pieces_to_pc_num(extra_pieces - 1, 5)}")
